...
## Six outputs at a glance (R17++ #6 restructure 2026-05-17 — boost strategy + revised venues + new data acquisition plan)

**R17++ #6 strategic pivot:** R17++ #5's venue targets were over-ambitious in several places (P1 EPJ DS, P3 JGME, P4 Scientific Data, P5 JAMA Surgery, P6 JMIR Formative all had fit problems). R17++ #6 boost strategy: (a) elevate 3 papers via reframing/new data; (b) downgrade 3 papers to better-fit venues; (c) add ~5-6 weeks of new data acquisition (all free public data). See `DATA_ACQUISITION_PLAN_R17pp6.md` for complete per-paper data needs.

## R17++ #5 outputs table (preserved for reference; superseded by R17++ #6 elevations below)
...
| **R17++ #6 (boost strategy + data acquisition plan)** | **2026-05-17 final** | After R17++ #5 rigorous validity review surfaced that venue targets were over-ambitious in 5 of 6 papers, user asked: "reevaluate rigorously the goals of each paper and provide realistic targets; or otherwise suggest how i can boost the project to make acceptance more likely" + "what other data do i need to pull for each? make these updates". Result: comprehensive **boost strategy with venue revisions + new data acquisition plan**. Three papers elevated via reframing + new data: (P1) reframe headline to OP-vs-Reply directional split → **ICWSM '26 / CSCW '26 / *IC&S*** primary; (P2) strip to Reddit Finance cross-scorer sign-flip → ***Political Analysis*** primary; (P3) methods translation + workforce-downstream addition via CMS NPPES → ***Health Affairs Scholar*** primary. Three papers downgraded to better-fit venues: (P4) Scientific Data with broadened "comprehensive residency-program characterization" framing + Data in Brief firm fallback; (P5) ***Neurosurgery* (Wolters Kluwer)** primary (down from JAMA Surgery reach); (P6) ***PLOS One*** primary (down from JMIR Formative scope mismatch). NEW data acquisition plan documented in **`DATA_ACQUISITION_PLAN_R17pp6.md`** — 7 free public data sources (CMS NPPES + ACGME multi-year + NIH RePORTER multi-year + VA designation + HRSA HPSA + USDA RUCA + KFF Medicaid + additional Reddit r/StudentLoans Arctic Shift); total ~5-6 weeks of data engineering during research year; total cost $0. Drafting order unchanged: P3 → P4 → P5 → P6 → P1 → P2. **Honest realistic ERAS yield with boost strategy: ~3.8-4.5 papers + ~4 abstracts = ~8-9 ERAS research items** (up from R17++ #5's ~5-6; NS match avg ~18; project contributes ~45-50%). New artifact: `DATA_ACQUISITION_PLAN_R17pp6.md`. **Honest acknowledgment**: This is the fourth iteration on venue + scope; each iteration improves as I get more rigorous review. The boost strategy is the final stable plan barring further user input. |
