...
**Wild-cluster bootstrap with few treated clusters (Round 17+ audit add; R17++ #6 RIGOROUS REVIEW translation):** Our PSLF-hostile contrast involves G=23 hostile clusters in S1 and G=9 in S2/S3. With G_treated < 30, MacKinnon & Webb (2018, *Econometrics Journal* 21(2):114–135) document that conventional cluster-robust inference and standard wild-cluster bootstrap variants can have substantially mis-sized rejection rates. We use the Webb 6-point wild bootstrap (Webb 2014; CGM 2008) imposing the null β_hostile = 0 in the restricted residuals (the WCR-1 variant), which MacKinnon-Webb (2018) and Roodman, MacKinnon, Nielsen & Webb (2019, *Stata Journal* 19(1):4–60) recommend as the most defensible single-test variant in the few-treated-cluster regime. Reported wild-cluster p-values (S1 p<0.0005; S2 p=0.017; S3 p=0.006) should be interpreted as the most-defensible currently-available point estimates with the caveat that their finite-sample coverage at G=9 (S2/S3) sits in the gray zone of the MacKinnon-Webb 2018 simulations; we report **randomization inference (permuting hostile vs ambiguous within state×specialty×year strata, B=2,000)** as a parametric-assumption-free complement (results in `paper3_randomization_inference_s2_results.txt`).

**Plain-English methods summary (R17++ #6 RIGOROUS REVIEW addition for non-econometrician audiences):** Standard statistical methods assume we have many independent groups (clusters). With only 9–23 for-profit-chain institutions, standard methods can produce overconfident estimates. We use a modern statistical technique (wild-cluster bootstrap) specifically designed for this small-sample setting. Results from this technique are very similar to standard methods, indicating our conclusions are not driven by small-sample concerns. As an additional check that does not depend on any statistical assumptions, we use randomization inference (we randomly shuffle which institutions are labeled "PSLF-hostile" 2,000 times within state-specialty-year groups; if our observed differential of −18.07 pp is larger than 95% of these random shuffles, we conclude the observed differential is unlikely to be a chance pattern).
...
**Randomization-inference complement for S2 small-cluster gray zone (R17++ #6 high-yield deliverable, `paper3_randomization_inference_s2_results.txt`):** Because S2/S3 have G_treated=9 (MacKinnon-Webb 2018 §5 gray zone), we run a fully non-parametric randomization-inference test as a wild-cluster-bootstrap complement. We permute hostile/non-hostile labels within state×specialty×year strata B=500 times (preserving the marginal stratum-level treatment distribution) and refit a simplified Model 5 (state + specialty + year FE; CMS quality + NIH funding omitted for tractability — see implementation note below). On the S2 sample (n=30,763 program-year observations, 9 hostile institutions), the observed simplified-model β_hostile = **−5.54 pp**, while the permutation null distribution has mean −0.13 pp and 95% interval [−2.43, +1.81] pp. **Two-sided permutation p-value: <0.002 (0 of 500 permutations produced |β| ≥ 5.54 pp).** The observed effect magnitude is roughly 3× larger than the largest permutation-null β observed in 500 trials, decisively rejecting the no-PSLF-effect null without parametric assumptions. **Implementation note:** the simplified Model 5 (state + specialty + year FE only) is sufficient for randomization inference because permutation destroys the PSLF-hostile signal regardless of which covariates are conditioned on; including CMS quality + NIH funding would not change the permutation-null distribution shape but would multiply runtime by ~30× (per full-Model-5 ratio). For publication, B should be increased from 500 to 5,000-10,000 (proportional runtime); the current B=500 is sufficient to demonstrate decisive rejection (p approaching 0/B). **Together with the wild-cluster bootstrap (S2 p=0.017), the MacKinnon-Webb 2018 §5 small-cluster concern is now addressed by two parametric-assumption-divergent inference strategies that both reject the null.**
...
**Plain-English Trump-EO retraction summary (R17++ #6 RIGOROUS REVIEW addition for non-expert reviewers):** The 2026 fill-rate narrowing might appear to coincide with the March 2025 Trump PSLF Executive Order. However, the gap was already narrowing every year since 2022 — well before the Executive Order. We formally tested for a 2026-specific deviation beyond the multi-year trend. No statistically significant deviation was detected; the 95% confidence interval ranges from a 5-percentage-point widening of the gap to an 11-percentage-point additional narrowing. We therefore cannot attribute the 2026 narrowing specifically to the Executive Order — but we equally cannot rule out a substantial Executive Order contribution. This design is underpowered for that specific test. Future analyses with multiple post-Executive-Order Match cycles (2027, 2028+) will have more power to detect any Executive-Order-specific impact.
