...
1. **Genuinely novel sub-finding (corrected per R17++ #6 review)**: **All 14 PSLF-hostile surgical-subspecialty program-institution pairs are HCA Healthcare-affiliated. These 14 pairs represent 8 unique HCA-affiliated facilities** (R17++ #6 review caught a double-counting error in earlier framing — see `paper5_mann_whitney_surgery_general.txt` for verification). HCA Healthcare Kansas City alone hosts 5 surgical training programs (Neurological Surgery, Plastic Surgery, Otolaryngology, Orthopedic Surgery, Surgery-General) — making it the most concentrated multi-subspecialty PSLF-hostile training site in the US.

2. **Workforce-policy descriptive finding (corrected per R17++ #6 review — Mann-Whitney NULL RESULT)**: For ultra-competitive surgical subspecialties (Vascular, Thoracic, Plastics, NS, ENT) hostile-program fill rate is 100% every year. **For Surgery-General, the only surgical subspecialty with adequate n_hostile (n=7 institutions, 110 program-years) for inferential testing, Mann-Whitney U comparing hostile vs non-hostile fill rates returns p=0.84 NS at program-year level (hostile mean 0.806 vs non-hostile mean 0.824) AND p=0.84 NS at institution level (hostile mean 0.832 vs non-hostile mean 0.841).** The earlier outline claim that "PSLF eligibility may meaningfully affect Surgery-General recruitment" was UNSUPPORTED by the data. **Honest reframe:** Surgery-General hostile programs show greater fill-rate variability than ultra-competitive subspecialties (range 0–1.0 vs 100% for competitive subspecs), but central tendency does not differ. This null result is itself informative for workforce-policy discussions.
...
**Design:** Cross-sectional analysis of NRMP Main Match Program Results 2021–2026 (n=7,614 surgical-subspecialty program-years from 7 subspecialties across **358 unique institutions**; the value "979" reported in earlier outline iterations was the SUM of per-subspec institution counts, which double-counts institutions hosting multiple surgical subspec programs — corrected per R17++ #6 review). PSLF eligibility classified using ProPublica IRS Form 990 verification (PSLF-eligible if 501(c)(3) or government; ambiguous if private/LLC; PSLF-hostile if for-profit chain-affiliated).
...
**Results:** Surgical-subspecialty PSLF-hostile rates ranged from 0% (Vascular, Thoracic) to 3.05% (Surgery-General). **All 14 PSLF-hostile surgical-subspecialty program-institution pairs are HCA Healthcare-affiliated, representing 8 unique HCA facilities** (R17++ #6 review correction; earlier framing of "14 unique institutions" double-counted facilities hosting multiple surgical-subspec programs). HCA Healthcare Kansas City alone hosts 5 surgical training programs (NS, Plastics, ENT, Ortho, Surgery-General), all PSLF-hostile, all at 100% fill every year. For ultra-competitive subspecialties (Vascular, Thoracic, Plastics, NS, ENT) PSLF-hostile programs maintain 100% fill, suggesting PSLF eligibility does not deter applicants for highly-competitive surgical training. **For Surgery-General, the only subspecialty with adequate n_hostile for inferential testing (n=7 institutions, 110 program-years), Mann-Whitney U returns p=0.84 NS at both program-year and institution levels** (hostile mean fill 0.806 vs non-hostile 0.824). Surgery-General hostile programs show greater fill-rate variability than competitive subspecialties (range 0–1.0), but central tendency does not differ from non-hostile Surgery-General programs.
...
**Data:** NRMP 2021–2026 (n=7,614 surgical-subspecialty program-years from 7 subspecialties across **358 unique institutions**); ProPublica IRS Form 990 PSLF eligibility classification per `DATASET_DEPOSIT_PLAN.md` (Dryad/Zenodo DOI to cite at submission). Three sensitivity specifications (S1/S2/S3) for HCA-academic partnerships — note: 0 surgical-subspecialty hostile programs are HCA-academic partnerships, so S1/S2/S3 identical for this analysis.

**Methodological caveat (R17++ #6 review):** The "all 14 PSLF-hostile surgical-subspec program-institution pairs are HCA Healthcare-affiliated" finding is conditional on the surgical-subspecialty slice. Across all 28 NRMP specialties (Paper 3), the full PSLF-hostile pool of 23 institutions includes 2 non-HCA institutions (North Oaks Med Ctr LLC and Steward Carney Hospital) that do NOT host surgical-subspecialty residencies. The all-HCA finding for surgical subspecialties is therefore not selection-on-outcome but reflects the actual data structure: HCA Healthcare's GME footprint specifically includes surgical-subspec programs whereas the other for-profit-chain hostiles do not.

**Statistical analyses:** Descriptive cross-subspecialty comparison of PSLF-eligibility rates + fill rates by PSLF class. For Surgery-General (only subspecialty with n_hostile≥5 institutions), Mann-Whitney U comparing hostile vs non-hostile fill rates: **U=193,974, p=0.8426 NS at program-year level; U=1,054.5, p=0.8427 NS at institution level** (see `paper5_mann_whitney_surgery_general.txt` for full output). Welch's t complement: t=−0.510, p=0.6111 NS. No formal regression given small n_hostile (= 7 institutions) in Surgery-General and zero or very small n_hostile in other surgical subspecs.
...
2. **HCA Healthcare as the single source of PSLF-hostile surgical-subspecialty training:** All 14 PSLF-hostile surgical-subspecialty program-institution pairs across our 6-year window are HCA Healthcare-affiliated, representing 8 unique HCA facilities. HCA Kansas City hosts the most concentrated multi-subspecialty PSLF-hostile training site (5 programs including NS, Plastics, ENT, Ortho, Surgery-General). The institutional concentration of for-profit-chain surgical-subspecialty training warrants attention from surgical-workforce policymakers and ACGME-accreditation reviewers. *Caveat (Methods): the all-HCA finding is conditional on the surgical-subspec slice; 2 non-HCA hostile institutions (North Oaks, Steward Carney) exist in the broader 23-institution all-specialty pool (Paper 3) but happen not to host surgical-subspec programs.*

3. **PSLF eligibility does NOT measurably affect Surgery-General fill rates** (R17++ #6 review correction). Mann-Whitney U test at both program-year and institution levels returns p=0.84 NS (hostile mean 0.806 vs non-hostile mean 0.824 at program-year; 0.832 vs 0.841 at institution-level). The earlier outline claim that "PSLF eligibility may meaningfully affect Surgery-General recruitment" was UNSUPPORTED by the inferential test we proposed in Methods. **Honest reframe:** Surgery-General hostile programs show greater fill-rate variability than competitive subspecialties (range 0–1.0 vs ≈1.0), but central tendency does not differ from non-hostile Surgery-General programs. This null result is itself informative for workforce-policy: at the surgical-subspecialty level, the cross-specialty −18 pp PSLF-hostile fill-rate differential in Paper 3 (companion publication) is driven primarily by Internal Medicine + Family Medicine + Emergency Medicine recruitment dynamics — surgical-subspecialty fill rates appear less sensitive to PSLF-eligibility classification at observable scales of analysis.
